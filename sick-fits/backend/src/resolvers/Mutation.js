...
const Mutations = {
  async createItem(parent, args, ctx, info) {
    ...
    const item = await ctx.db.mutation.createItem({
      data: {
        user: {
          // This is how to create a rtelationship between item and user:
          connect: {
  ...
  async signup(parent, args, ctx, info) {
    ...
    // Hash their passwords with random salt of length 10
  ...
  async requestReset(parent, args, ctx, info) {
    ...
    const resmut = await ctx.db.mutation.updateUser({
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    // 3. Check if the item is already in their cart and, if it is, increment count
    if (existingCartItem) {
    ...
    // 4. If its not, create a fresh cart item
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    if (cartItem.user.id !== ctx.request.userId)
      throw new Error('Cheting huhhhh!');
